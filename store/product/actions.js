export default {
  async fetchLists({ commit, state }) {
    ...
    await this.$axios.get('/product', { params: listParams }).then((res) => {
  ...
  async createModel({ commit }, payload) {
    return await this.$axios.post('/product', payload)
  },

  async updateModel({ commit, state }, payload) {
    if (payload) {
      return await this.$axios.put('/product/' + payload.id_product, payload)
    }
  },
